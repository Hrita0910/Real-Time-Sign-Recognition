...
# Variables for stable prediction and history
current_char = "None"
char_buffer = []
buffer_size = 3
hold_frames = 30
...
while cap.isOpened():
    ...
    if results.multi_hand_landmarks:
        for hand_landmarks in results.multi_hand_landmarks:
            ...
            # Predict ASL character using trained model
            if frame_counter <= 0:
...
# Print and speak detected characters
print("Detected Characters in Session (up to last 10):")
if char_history:
    detected_chars = " ".join(char_history)
    print(detected_chars)
    # Speak the detected characters
    engine.say("The detected characters are: " + detected_chars)
    engine.runAndWait()
else:
    print("No characters were detected during the session.")
    engine.say("No characters were detected during the session.")
    engine.runAndWait()
