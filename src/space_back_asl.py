...
def process_word_buffer(buffer):
    detected_words = []
    word_to_check = buffer.lower().replace('_', '')  # Remove placeholders for display
...
char_history = []
word_buffer = ""
detected_words = []  # For display purposes
...
while cap.isOpened():
    ...
    cv2.putText(canvas, "Current Buffer:", (10, words_start_y), 
                cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
    display_buffer = word_buffer.replace('_', ' ')
...
if detected_words:
    ...
    try:
        ...
        print(f"Speaking phrase in English: {detected_words_str}")
        engine.say(detected_words_str)
        engine.runAndWait()
        ...
        speak_bengali(bengali_words_str)
    except Exception as e:
    ...
    speak_bengali("কোনো শব্দ সনাক্ত করা হয় নি।")
