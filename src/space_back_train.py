...
def data_generator(X, y, batch_size, augment=True):
    while True:
        ...
        batch_X = batch_X.reshape(-1, 8, 8, 1)  # Adjusted for 64 features
        yield batch_X, batch_y

# Load dataset (ASL Alphabet Dataset with space and backspace)
...
for label in labels:
    ...
    for img_name in os.listdir(label_dir):
        ...
        if results.multi_hand_landmarks:
            for hand_landmarks in results.multi_hand_landmarks:
                ...
                img_count += 1
                if img_count % 10 == 0:
                    print(f"Processed {img_count} images for label {label}")
        else:
    ...
    print(f"Finished processing {img_count} images for label {label} (skipped {skipped_images} images)")

# Save image counts to file
os.makedirs('../outputs', exist_ok=True)
with open('../outputs/image_counts.txt', 'w') as f:
    f.write("Summary of Processed Images:\n")
    f.write("Label  | Valid Images | Total Images | Skipped Images\n")
    f.write("-" * 45 + "\n")
    for label in labels:
...
print("Label  | Valid Images | Total Images | Skipped Images")
print("-" * 45)
for label in labels:
    if image_counts[label]['skipped'] == -1:
...
# Build CNN model with adjusted output layer for 28 classes
model = tf.keras.Sequential([
    ...
    tf.keras.layers.Dense(len(labels), activation='softmax')  # Updated for 28 classes
])
...
model.save('models/asl_model_with_space_backspace.keras')
...
plt.savefig('outputs/training_history_with_space_backspace.png')

hands.close()
print("Training completed successfully!")
print("Image count summary saved to ../outputs/image_counts.txt")
print("Model saved to ../models/asl_model_with_space_backspace.keras")
print("Training history plot saved to ../outputs/training_history_with_space_backspace.png")